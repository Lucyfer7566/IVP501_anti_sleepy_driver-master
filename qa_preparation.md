...
#### Q23: Khi risk đạt 100, tài xế mở to mắt tỏ ra tỉnh táo nhưng risk vẫn delay một lúc rồi mới giảm — đây là lỗi hay tính năng?

**Trả lời:**
"Dạ đây là **tính năng an toàn có chủ đích**, không phải lỗi. Khi risk đạt 100 (CONFIRMED DROWSINESS), hệ thống có 2 cơ chế chống tắt cảnh báo sớm:

1. **PERCLOS inertia:** Cửa sổ PERCLOS dài 90 frame (~3 giây). Khi tài xế vừa mở mắt, các frame cũ (mắt nhắm) vẫn nằm trong buffer → PERCLOS vẫn > 0.30 → penalty +2.0/frame tiếp tục. Chỉ khi đủ frame mới (mắt mở) đẩy frame cũ ra khỏi buffer thì PERCLOS mới hạ.

2. **Risk decay rate = 1.0/frame:** Từ risk = 100 xuống dưới 50 (NORMAL) cần ít nhất 50 frame ≈ 1.7 giây **mà không có bất kỳ penalty nào**. Trong thực tế, PERCLOS penalty vẫn cộng thêm vài giây → tổng delay khoảng 2-3 giây.

Thiết kế này mô phỏng thực tế y khoa: người vừa microsleep xong dù mở mắt ra nhưng **phản xạ thần kinh chưa hồi phục ngay**, cần vài giây để tỉnh hoàn toàn. Nếu hệ thống tắt cảnh báo ngay khi mắt mở, tài xế có thể ngủ lại ngay lập tức mà không kịp dừng xe."

---

#### Q24: Khi thay tài xế (người mới ngồi vào), hệ thống mất bao lâu để tự điều chỉnh?

**Trả lời:**
"Dạ hệ thống có 2 giai đoạn tự điều chỉnh:

**Giai đoạn 1 — Tức thì (0-2 giây):** Cửa sổ trượt EAR gồm 60 frame. Khi tài xế mới ngồi vào, trong 2 giây đầu, các giá trị EAR cũ bị đẩy ra, thay bằng EAR mới. `dynamic_ear_open` — giá trị EAR mở tối đa — sẽ **nhảy lên ngay** nếu EAR mới cao hơn (update tức thì), hoặc **giảm từ từ** nếu mắt mới nhỏ hơn (decay factor 0.995). Sau khoảng 60 frame (~2 giây), `ear_thresh` tự calibrate xong với tài xế mới.

**Giai đoạn 2 — Liên tục (sau 2 giây):** `base_pitch` và `base_yaw` — tư thế đầu cơ sở — thích ứng liên tục qua EMA (Exponential Moving Average) với hệ số 0.995:0.005. Nghĩa là sau ~200 frame (~7 giây), tư thế cơ sở đã dịch chuyển 50% về phía tài xế mới.

Tổng cộng, sau **khoảng 2-3 giây**, hệ thống hoạt động chính xác với tài xế mới mà không cần bất kỳ thao tác đăng ký nào."

---

#### Q25: Khi quay đầu nhìn ngang (kiểm tra gương, nhìn hành khách) rồi quay lại, hệ thống có bị báo nhầm không?

**Trả lời:**
"Dạ đây là một **hạn chế đã biết** (known limitation) của approach dựa trên EAR 2D. Nhóm em xin giải thích nguyên nhân và hướng khắc phục.

**Nguyên nhân gốc:** EAR được thiết kế để đo tỷ lệ mắt khi nhìn **chính diện**. Khi tài xế quay đầu khoảng 30-45 độ, mắt phía xa camera bị **thu hẹp phối cảnh** (perspective foreshortening) — mắt vẫn mở nhưng trong hình 2D, các landmark bị dồn lại gần nhau, khiến EAR tụt rất thấp. Hệ thống hiểu nhầm đó là nhắm mắt.

Hệ thống hiện tại có **Veto Rules** — nếu yaw > 30° hoặc pitch < -25°, EAR penalty sẽ bị bỏ qua. Tuy nhiên, solvePnP dùng mô hình 6 điểm đơn giản nên yaw output **không tỷ lệ tuyến tính** với góc quay đầu thực tế. Khi quay 45° thật, solvePnP có thể chỉ báo 15-20° — dưới ngưỡng Veto.

**3 hướng khắc phục đã nghiên cứu:**
1. **Phát hiện bất đối xứng mắt:** Khi quay đầu, `|EAR_trái − EAR_phải| > 0.06` → tạm ngưng penalty. Không phụ thuộc solvePnP.
2. **Yêu cầu cả 2 mắt đều dưới ngưỡng:** Thay `avg_ear < thresh` bằng `left < thresh AND right < thresh`. Loại bỏ trường hợp 1 mắt méo.
3. **3D Face Reconstruction:** Tính EAR trên không gian 3D, triệt để nhưng tốn tài nguyên.

**Lý do chưa áp dụng:** Khi thử nghiệm hướng 1 và 2, hệ thống **giảm độ nhạy** với trường hợp gục đầu + nhắm mắt — trường hợp nguy hiểm nhất. Khi gục đầu, MediaPipe vẫn detect mặt nhưng cả 2 mắt đều bị biến dạng EAR tương tự nhau → không phân biệt được 'gục đầu ngủ gật' và 'quay ngang'. Đây là bài toán **Precision vs Recall** — nhóm chọn ưu tiên **Recall** (không bỏ sót) vì tính mạng con người quan trọng hơn vài lần báo động nhầm. Trong thực tế lái xe, tần suất quay đầu ngang 45° rất thấp so với tần suất nhắm mắt khi buồn ngủ."
